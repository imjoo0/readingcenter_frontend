...
//                   <S.TableHead style={{ width: "30%" }}>
//                     {
//                       bookCountData?.getLecturesBookCount?.filter((el) => {
...
export default function ClassPage() {
  ...
  const [firstAlarm, setFirstAlarm] = useState(false);
  ...
  // const [totalPages, setTotalPages] = useState(1);
  const totalPages = Math.ceil(
    (studentListData?.getLecturesByAcademyAndDateStudents?.length ?? 1) /
      itemsPerPage
  );
  ...
  useEffect(() => {
    ...
    students.sort((a, b) => {
      ...
      if (
        ...
        a?.attendanceStatus?.statusDisplay === "보강" ||
      ...
      if (
        ...
        b?.attendanceStatus?.statusDisplay === "보강" ||
    ...
    const start = (page - 1) * itemsPerPage;
    const end = start + itemsPerPage;
    setStudentArray(students?.slice(start, end));
    console.log(students);
  }, [studentListData, page]);
  ...
  const onChangeEach = (e, lectureId, studentId) => {
    if (e.target.checked) {
      ...
      console.log(e, lectureId, studentId, checkList, "check");
    } else {
      setCheckList(
        checkList.filter(
          (item) =>
  ...
  const checkTargetTime = (audio) => () => {
    const now = new Date();
    setCheckDate(now);
    console.log(router.query.branch, "now");
    // startTimes와 endTimes 배열을 로컬 스토리지에서 가져오기
      ...
      startTimes.forEach(async (startTime) => {
        if (
          (now.getHours() === startTime.getHours() &&
            now.getMinutes() === startTime.getMinutes()) ||
          (now.getHours() === startTime.getHours() &&
            now.getMinutes() === startTime.getMinutes() + 5)
        ) {
          // 원하는 동작 실행 (예: 알림 표시, WebSocket 메시지 전송 등)
          try {
            const result = await refetchAttendance({
            ...
            setAlarmType("start");
            console.log(result);
            if (result?.data?.getAttendance?.length !== 0) {
              console.log("play1");
              audio.play();
              console.log("play2");
            }
      ...
      endTimes.forEach(async (endTime) => {
        if (
          (now.getHours() === endTime.getHours() &&
            now.getMinutes() === endTime.getMinutes()) ||
          (now.getHours() === endTime.getHours() &&
            now.getMinutes() === endTime.getMinutes() - 5)
        ) {
          // 원하는 동작 실행 (예: 알림 표시, WebSocket 메시지 전송 등)
          try {
            const result = await refetchAttendance({
  ...
  useEffect(() => {
    ...
    if (router.query.branch !== undefined) {
      ...
      setInterval(checkTargetTime(audio), 60 * 1000);
    }
  }, [studentData]); // refetch
            ...
            return (
              <tr>
                ...
                <td>
                  <SearchOutlined
                    onClick={() => {
                      window.open("/2/academy/" + el.student.id);
                    }}
                  />
                </td>
              </tr>
